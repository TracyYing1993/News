 ...
 * 新闻收藏记录Activity
 */
public class NewsCollectRecordActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        table_title.setText("新闻收藏记录表");

        // 查出所有的记录数
        Cursor cursor1 = db.findSelectCursor(
                "select count(*) from news_record",
                SQLiteDatabaseUtils.NEWS_RECORD, null);
        cursor1.moveToFirst();
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        boolean result = db.executeSql(
                                SQLiteDatabaseUtils.NEWS_RECORD,
                                "delete from news_record", null);
                        if (result) {
            ...
            case R.id.delete:// 删除
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        boolean result = db.executeSql(
                                SQLiteDatabaseUtils.NEWS_RECORD,
                                "delete from news_record where _id=?",
                                new String[] { id });
                        if (result) {
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        Map<String, Object> map = (Map<String, Object>) adapter
                .getItem(position);
        int _id = Integer.parseInt((String) map.get("_id"));
        Intent intent = new Intent(this, NewsWebActivity.class);
        Bundle bundle = new Bundle();
        bundle.putInt("webUrl", _id);
        ...
        String create_time= (String) map.get("creat_time");
        //存储到记录表中
        long rows = db.addAllCloumnData(SQLiteDatabaseUtils.ACCESS_RECORD, _id + "",
                title, datas,create_time ,null,null,"1");
        if (rows > 0) {
